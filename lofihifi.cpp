    ...
    case WM_COMMAND:
        {
            ...
            case IDM_NEWDATA:
                ...
                for (unsigned int i = 0; i < hidd; i++) for (unsigned int j = 0; j < hidd; j++) {
                    //rnd = 196314165 * rnd + 907633515;  nn1[i][j] = .25f + (float)(rnd >> 17) / 65536.f;
                    //rnd = 196314165 * rnd + 907633515;  nn2[i][j] = .25f + (float)(rnd >> 17) / 65536.f;
                    //rnd = 196314165 * rnd + 907633515;  nn3[i][j] = .25f + (float)(rnd >> 17) / 65536.f;
                    //rnd = 196314165 * rnd + 907633515;  nn4[i][j] = .25f + (float)(rnd >> 17) / 65536.f;
                }
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            if (SUCCEEDED(hr)) {
                                ...
                                if (SUCCEEDED(hr)) {
                                    ...
                                    //infile.read(reinterpret_cast<char*>(&nn1), sizeof(nn1));
                                    //infile.read(reinterpret_cast<char*>(&nn2), sizeof(nn2));
                                    //infile.read(reinterpret_cast<char*>(&nn3), sizeof(nn3));
                                    //infile.read(reinterpret_cast<char*>(&nn4), sizeof(nn4));
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            if (SUCCEEDED(hr)) {
                                ...
                                if (SUCCEEDED(hr)) {
                                    ...
                                    //out.write(reinterpret_cast<char*>(&nn1), sizeof(nn1));
                                    //out.write(reinterpret_cast<char*>(&nn2), sizeof(nn2));
                                    //out.write(reinterpret_cast<char*>(&nn3), sizeof(nn3));
                                    //out.write(reinterpret_cast<char*>(&nn4), sizeof(nn4));
                ...
                while (tests < 100000) {
                    ...
                    if (dBcheck >> 8) {
                        ...
                        if (!(tests & 15)) {
                            ...
                            if (stop) { tests = 999999;   stop = 0; }
                        }
                        tests++;
                    }
                    testpos += teststep;    if (testpos > samples - 520) testpos -= (samples - 520);
                }
                if (tests == 100000) epochs++;
                tests = 0; 
            }   break;

            case IDM_TRAIN10X: {
                loaded = 6;
                for (int do10x = 0; do10x < 10; do10x++) {
                    //float nnh[420];	//	hidden layer
                    //float nnhe[420];//	hidden error
                    //float nnoe[180];//	output error
                    unsigned long int teststep = (unsigned long int)((float)(samples - 520) / (float)numtests);
                    ...
                    while (tests < 100000) {
                        ...
                        if (dBcheck >> 8) {
                            ...
                            if (!(tests & 15)) {
                                ...
                                MSG tmsg;
                                while (PeekMessage(&tmsg, hWnd, 0, 0, PM_REMOVE)) { TranslateMessage(&tmsg); DispatchMessage(&tmsg); }
                                if (stop) { tests = 999999;   do10x = 10;   stop = 0; }
                            }
                            tests++;
                        }
                        testpos += teststep;    if (testpos > samples - 520) testpos -= (samples - 520);
                ...
                while (wavpos < samples) {
                    ...
                    for (unsigned int i = 0; i < hidd; i++) { //  size of hidden..    input to hidden
                        ...
                        for (int j = 1; j < 117; j++) { //  input is 232, 116 mag bins 116 phase bins
                            hsum += wmag[j] * 0.2f * nni[j - 1][i];
                            hsum += wpha[j] * 0.2f * nni[j + 115][i];
                        }
                        nnh[i] = std::tanh(hsum);
                    }
                    ...
                    for (unsigned int i = 0; i < 90; i++) { //  size of output/2 (split) hidden to output
                        ...
                        nnr[i] = std::tanh(osum);
                        float st = nnr[i];
                        if (st > 0.76f) safe += st * st;//.4
                        wmag[m] = (nnr[i] - 0.4f) * 5.f;
                        osum = 0.f;
...
//next try squeezing .36 bias to .4, try making safe functional again



            if (safe > 0.8f) {//catches bad scary wrong stuff .4
                float s2 = 0.f;
                for (int i = 1; i < 117; i++) s2 += wmag[i];
                safe /= s2 * .00001f;
                safearr[safect] = 100 + (unsigned long int)(800.f * (float)wavpos / (float)samples);
                shadarr[safect] = (unsigned char)(safe + 64.f);

                //safe *= .00001f;//.00005f;
                
                safe = fmax(fmin(safe, 4.f), .5f);//8.f,2.f
                safe = 0.f;
                //safe < 2.f ? safe = .5f : safe = 1.f / safe;
                //for (int i = 117; i < 207; i++) if (wmag[i] > .6f) wmag[i] = fmax(fmin(.9f, wmag[i]) - safe, -8.f);//.25,1.,-8.
                for (int i = 117; i < 207; i++) if (wmag[i] > .6f) wmag[i] = fmax(fmin(.9f, wmag[i]) - safe, -8.f);
                safect++;
            }


            float edgesum = wmag[110] + wmag[111] + wmag[112] + wmag[113] + wmag[114] + wmag[115] + wmag[116];
            edgesum *= .142857f;
            for (unsigned int i = 1; i < 16; i++) {
                unsigned int j = i + 116;   float smooth = (float)i / 16.f;
                smooth = edgesum + smooth * (wmag[j] - edgesum);
                if (wmag[j] > smooth) wmag[j] = smooth;
            }




                //    for (unsigned int i = 0; i < 25; i++) wmag[i + 182] *= atten[i];
                    for (unsigned int i = 117; i < 207; i++) {
                        float tm = std::pow(10.f, wmag[i]);  //  second term is inverted as a condition of IFFT.. FFT>(rx,ix), (rx,-ix)>IFFT
                    ...
                    //
                    /*if (wavpos == 726784) {//151808  726784  1100288 1690112 1193728
                        std::ofstream outfile("magsneg.txt", std::ios::out); // out = text instead of binary
                        for (unsigned long int i = 117; i < 207; i++) {
                            char str[32];   sprintf(str, "%f", wmag[i]);
                            outfile.write(reinterpret_cast<char*>(&str), sizeof(str));
                            outfile << std::endl;
                        }
                        outfile.close();
                    }*/
                ...
                float a0, a1, a2, a3;			//	biquads + various
                float c0, c1, c2, c3;
                float d0, d1, d2, d3;		//	state (buffer) variables for bilinear integrator filters
                float g0, g1, g2, g3, g4, g5;
                float ifp1, nt;					//	neotec, bilinear integrator

                float f = pi * 15000.f / 44100.f;  f = std::tan(f); //  takes half ang. freq
                float q = 0.01f;
                d0 = d1 = d2 = d3 = 0.f;
                a0 = a1 = a2 = a3 = 0.f;
                c0 = c1 = c2 = c3 = 0.f;
                nt = f + 1.f;	ifp1 = 1.f / nt;
                g0 = nt * nt;	g1 = f * g0;	g0 *= nt;	g2 = f * f;
                g3 = f * g2;	g4 = g2 * g2;	g2 *= nt;	g5 = g0 * g0 + g4 * q;
                g0 /= g5;	g1 /= g5;	g2 /= g5;	g3 /= g5;	g4 /= g5;

                for (unsigned long int i = 0; i < samples; i++) {   //  24dB lowpass "zero delay feedback filter" by neotec
                    float o = d3 * g0 + d2 * g1 + d1 * g2 + d0 * g3 + wav1[i] * g4;
                    float o1 = wav1[i] - o * q;
                    a0 = o1 - (f * o1 + d0) * ifp1;	c0 = d0 + f * a0;	d0 = c0 + c0 - d0;
                    a1 = c0 - (f * c0 + d1) * ifp1;	c1 = d1 + f * a1;	d1 = c1 + c1 - d1;
                    a2 = c1 - (f * c1 + d2) * ifp1;	c2 = d2 + f * a2;	d2 = c2 + c2 - d2;
                    a3 = c2 - (f * c2 + d3) * ifp1;	c3 = d3 + f * a3;	d3 = c3 + c3 - d3;
                    wav0[i] = d3 + o * q;
                }
                ...
                for (int i = 0; i < safect; i++) for (unsigned int ii = 190; ii < 209; ii++) SetPixel(hdcbackground, safearr[i], ii, RGB(shadarr[i], 200, 0));
    ...
    case WM_CREATE:
    {
        ...
        for (unsigned int i = 0; i < 512; i++) { hann[i] = (.5f - .5f * cos(tix * (float)i)) * 64.f; }
     //   for (unsigned int i = 0; i < 512; i++) hann[i] = (.426591f - .49656f * cos(tix * (float)i) + .076849f * cos(tix * (float)(i + i))) * 64.f;
        ...
        for (int i = 0; i < 25; i++) { float t = (float)i * .038f;    atten[i] = 1.f - t * t; }
    ...
    case WM_LBUTTONDOWN: {
        ...
        else if (mouloc.x < 90 && mouloc.y > 240) {
            if (mouloc.y > 410) {
                int t = mouloc.y - 400; t >>= 5;    dreg = min((unsigned char)t, 3);    regs = rreg[dreg];
            }
            else {
                int t = mouloc.y - 240; t >>= 5;    dlearn = min((unsigned char)t, 4);   learn = rlearn[dlearn];
            }
        }
    }   break;
    case WM_PAINT:
       ...
       // TextOutA(hdcframe, 20, 16, "Open wav", 8);
        ...
        switch (loaded) {
            case 0: 
                TextOutA(hdcframe, 125, 29, "for 44100 samplerate files with spectra to 10kHz (riffusion)", 60);
                TextOutA(hdcframe, 125, 49, "left channel of stereo files used for processing", 48);
                TextOutA(hdcframe, 125, 89, "loop removal function requires exact length and takes forever", 61);
                TextOutA(hdcframe, 125, 109, "reducing the range will help but just don't do it", 50);
                TextOutA(hdcframe, 125, 149, "to enhance audio:", 17);
                TextOutA(hdcframe, 125, 169, "file > open wav, options > open data, options > apply hifi", 58);
                TextOutA(hdcframe, 125, 189, "enhanced file is half amplitude to accomodate peaks", 51);
                TextOutA(hdcframe, 125, 209, "recommended to train model on music similar to goal", 51);
                TextOutA(hdcframe, 125, 229, "until generated peaks do not exceed training material.", 54);

                TextOutA(hdcframe, 125, 269, "to train:", 9);
                TextOutA(hdcframe, 125, 289, "options > new data or use previous data model", 45);
                TextOutA(hdcframe, 125, 309, "regularisation (reg) makes training jump out of overfitting", 59);
                TextOutA(hdcframe, 125, 329, "select lower training coefficients as epochs advance", 52);
                TextOutA(hdcframe, 125, 349, "training uses a random offset of 0 to 255 samples each session, so", 66);
                TextOutA(hdcframe, 125, 369, "a few minutes of audio provides millions of training instances.", 63);
                TextOutA(hdcframe, 125, 389, "training on audio with just bass encourages lower peaks and noise.", 66);

                TextOutA(hdcframe, 125, 429, "open wav to begin", 17);  break;
            ...
            case 3: {   //  perform fourier analysis at position
                TextOutA(hdcframe, 72, 250 + (dlearn << 5), "<", 1);
                TextOutA(hdcframe, 72, 410 + (dreg << 5), "<", 1);
            ...
            case 6: {   //  training
                ...
                sprintf(str, "mag:   %f", readm);    TextOutA(hdcframe, 880, 320, str, strlen(str));
                sprintf(str, "pha:   %f", readph);    TextOutA(hdcframe, 880, 340, str, strlen(str));


                sprintf(str, "learn: %.5f", learn);    TextOutA(hdcframe, 880, 480, str, strlen(str));
                sprintf(str, "reg:   %1.0e", regs);    TextOutA(hdcframe, 880, 500, str, strlen(str));

                if (!tests || tests > 999) {
                    float errmean = merr + perr; tests ? errmean /= (float)tests : errmean *= 1e-9f;//-5 for 5 layer
                    sprintf(str, "mean:   %f", errmean);    TextOutA(hdcframe, 880, 440, str, strlen(str));

                    tests ? errmean = merr / (float)tests : errmean = merr * 1e-9f;
                    sprintf(str, "m mean:   %f", errmean);    TextOutA(hdcframe, 880, 380, str, strlen(str));
                    tests ? errmean = perr / (float)tests : errmean = perr * 1e-9f;
                    sprintf(str, "p mean:   %f", errmean);    TextOutA(hdcframe, 880, 400, str, strlen(str));
                }
                ...
                for (int i = 0; i < 90; i++) {
                    int j = i + 117;
                    SetPixel(hdcframe, logbin[j] - 206, 360 - (int)((nnr[i] - 0.4f) * 160.f), RGB(250, 100, 0));  //  *32
            ...
            case 7: {   //  applying hifi
                ...
                for (int i = 0; i < 90; i++) {
                    int j = i + 117;
                    SetPixel(hdcframe, logbin[j] - 206, 360 - (int)((nnr[i] - 0.4f) * 160.f), RGB(250, 60, 60));  //  *32
